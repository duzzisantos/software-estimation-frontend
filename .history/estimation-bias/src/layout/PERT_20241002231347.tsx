...
const PERTAnalysis = ({ selectedTasks, setSelectedTask }: FormSelection) => {
  ...
  const handleTaskValues = (newTask: string, newValue: number) => {
    setPopulate((prev: any) => {
      if (prev.newTask.includes(newTask)) {
        return prev.filter((el: any) => el.task !== newTask);
      } else {
        return [...prev, { newTask, newValue }];
      }
    });
  };

  const handleChange = (e: { target: { value: string } }) => {
    setTaskValue(parseFloat(e.target.value));
  };
  return (
          ...
          {selectedTasks.map((taskName, index) => (
            <Form.Group key={index} className="mb-2 col-lg-2 col-sm-8 col-md-8">
              ...
              <Form.Control
                ...
                onChange={(e) => {
                  handleChange(e);
                  handleTaskValues(taskName, taskValue);
                }}
              />
            </Form.Group>
          ))}
