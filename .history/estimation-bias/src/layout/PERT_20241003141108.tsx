...
const PERTAnalysis = ({ selectedTasks, setSelectedTasks }: FormSelection) => {
  ...
  return (
    <Col className="my-4 p-4">
      ...
      <Col className="bg-primary-subtle p-3 hstack gap-5">
        <output>
          Optimistic Estimation: <Badge bg="success">{100}</Badge>
        </output>
        <output>
          Most Likely Estimation: <Badge bg="info">{100}</Badge>
        </output>
        <output>
          Pessimistic Estimation: <Badge bg="danger">{100}</Badge>
        </output>
      </Col>
    </Col>
  );
